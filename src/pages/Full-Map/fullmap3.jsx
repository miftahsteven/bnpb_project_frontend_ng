...
export default function FullMap() {
    ...
    // Store data rambu untuk re-process saat options loaded
    const [rambuData, setRambuData] = useState([]);
    
    // Ref untuk akses provOptions terbaru di dalam closure map event/init
    const provOptionsRef = useRef(provOptions);
    useEffect(() => { provOptionsRef.current = provOptions; }, [provOptions]);
    ...
    useEffect(() => {
        ...
        const map = new maplibregl.Map({
            ...
            // center: [118.0, -2.5],
            // zoom: 3,
            bounds: [[92.0, -14.0], [145.0, 10.0]], // Widen bounds to zoom out
        ...
        map.once("load", async () => {
            ...
            map.addLayer({
                ...
                minzoom: 6, // Show only when zooming in >= 6
                paint: {
                    // radius tetap
                    "circle-radius": 5,
            ...
            // 4) Province Count Group (Low Zoom < 6)
            map.addSource("province-count-source", { type: "geojson", data: { type: "FeatureCollection", features: [] } });

            // Circle background
            map.addLayer({
                id: "province-count-circle",
                type: "circle",
                source: "province-count-source",
                maxzoom: 6, // Show only when zooming out < 6
                paint: {
                    "circle-radius": 25,
            ...
            // Popup Aggregation (Province)
            const provPopup = new maplibregl.Popup({ closeButton: true, closeOnClick: true });
            map.on("click", "province-count-circle", (e) => {
                const f = e.features?.[0];
                if (!f) return;
                
                const props = f.properties;
                const stats = JSON.parse(props.district_stats || "[]");
                
                const listHtml = stats.slice(0, 10).map(s => 
                    `<li class="d-flex justify-content-between align-items-center" style="font-size:12px; margin-bottom:4px">
                        <span>${s.name}</span>
                        <span class="badge bg-secondary rounded-pill">${s.count}</span>
                    </li>`
                ).join('');
                
                const more = stats.length > 10 ? `<li class="text-center text-muted" style="font-size:11px">... +${stats.length - 10} kecamatan lainnya</li>` : '';

                const html = `
                    <div style="min-width: 240px; max-height: 320px; overflow-y: auto; padding: 4px;">
                        <h6 class="mb-2 border-bottom pb-2">${props.province}</h6>
                        <div class="mb-2" style="font-size:13px">Total: <b>${props.count}</b> Rambu</div>
                        
                    </div>
                `;
                
                provPopup.setLngLat(f.geometry.coordinates).setHTML(html).addTo(map);
            });
            map.on("mouseenter", "province-count-circle", () => (map.getCanvas().style.cursor = "pointer"));
            map.on("mouseleave", "province-count-circle", () => (map.getCanvas().style.cursor = ""));
        });
        ...
        async function refreshRambu() {
            try {
                ...
                if (mapRef.current) {
                    const { features, provFeatures } = prepareMapsData(points);
                    const src = mapRef.current.getSource("rambu-source");
                    if (src) src.setData({ type: "FeatureCollection", features });
                    const provSrc = mapRef.current.getSource("province-count-source");
                    if (provSrc) provSrc.setData({ type: "FeatureCollection", features: provFeatures });
    ...
    const handleApplyFilter = async () => {
        ...
        const provSrc = map?.getSource("province-count-source");
        if (provSrc) provSrc.setData({ type: "FeatureCollection", features: provFeatures });
    ...
    // Re-process rambu data when provOptions change (e.g., after they are loaded)
    useEffect(() => {
        const map = mapRef.current;
        if (!map || !map.isStyleLoaded?.() || rambuData.length === 0) return;

        console.debug('[FullMap] Re-processing rambu data due to provOptions change');
        const { features, provFeatures } = prepareMapsData(rambuData);
    ...
    }, [rambuData, provOptionsRef.current]);

    // Helper untuk memproses data rambu ke Features Map & Aggregation
    function prepareMapsData(points) {
        const features = [];
        const provGroups = {};
        ...
        (Array.isArray(points) ? points : []).forEach((p) => {
            ...
            features.push({
                ...
                properties: { id: p.id ?? `${lon},${lat}`, title: p.name ?? "Rambu", categoryId: p.categoryId ?? p.category_id ?? null, ...p },
            });

            // Aggregate
            const pId = p.prov_id || p.province?.id || "unknown";
            
            // Cari nama provinsi (Prioritas: provOptionsRef > Object > String)
            let pName = null;
            const opts = provOptionsRef.current || [];
            if (opts.length > 0) {
                    const opt = opts.find(o => String(o.value) === String(pId));
                    if (opt) pName = opt.label;
            }

            if (!pName) {
                if (typeof p.province === 'object' && p.province?.name) {
                    pName = p.province.name;
                } else if (typeof p.province === 'string' && isNaN(Number(p.province))) {
                    pName = p.province;
                }
            }
            if (!pName) pName = `Provinsi ${pId}`;

            // District Name
            let dName = p.district?.name || (typeof p.district === 'string' ? p.district : null) || p.district_name || "Unidentified";
            if ((!dName || !isNaN(Number(dName))) && p.district_id) {
                    dName = `Kecamatan ${p.district_id}`;
            }

            if (!provGroups[pId]) {
                provGroups[pId] = { count: 0, sumLat: 0, sumLon: 0, name: pName, districts: {} };
            }
            const g = provGroups[pId];
            
            // Update nama jika sebelumnya fallback
            if (g.name.startsWith("Provinsi ") && !pName.startsWith("Provinsi ")) {
                g.name = pName;
            }
            ...
            if (!g.districts[dName]) g.districts[dName] = 0;
            g.districts[dName]++;
        });

        // Calculate Average
        const groupValues = Object.values(provGroups);
        const avg = groupValues.length > 0 ? (groupValues.reduce((a, b) => a + b.count, 0) / groupValues.length) : 0;

        const provFeatures = groupValues.map(g => {
                let color = "#FFA500"; 
                if (g.count > avg * 2.0) color = "#8B0000";      
                else if (g.count > avg * 1.5) color = "#D2691E"; 
                else if (g.count > avg * 0.8) color = "#FF8C00"; 
                else color = "#FFB74D";                          

                const stats = Object.entries(g.districts)
                ...
                geometry: {
                    type: "Point",
                    coordinates: [g.sumLon / g.count, g.sumLat / g.count]
                },
                properties: {
                    count: String(g.count),
                    province: g.name,
                    color: color,
                    district_stats: JSON.stringify(stats)
                }
            };
        });

        return { features, provFeatures };
    }
